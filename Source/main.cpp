...
bool filter_adult(const Person &p) { return p.age > 18; }

void proj_names(Person *p, Name *out) { std::memcpy(&out->name, &p->name, 50); }
...
void print_people(char *data) {
  ...
  std::cout << p->delta.ts << " " << p->delta.count << " " << p->data.name
            << " " << p->data.surname << " " << p->data.age << std::endl;
}
...
int main() {
  ...
  std::string test_str = "insert " +
  std::to_string(AliceDB::get_current_timestamp()) +   " Alice Peczyk 25";
  parseLine(test_str, alice);
  std::cout<<alice->delta.ts << " " << alice->delta.count << " "
  <<alice->data.name << " " << alice->data.surname << " " << alice->data.age <<
  std::endl;
  */
  ...
  std::array<std::string, 100> names = {
      ...
      "Pamela",      "Scott",     "Emma",      "Brandon",   "Nicole",
      "Frank",       "Samantha",  "Benjamin",  "Katherine", "Gregory",
      "Christine",   "Samuel",    "Debra",     "Raymond",   "Rachel",
      "Patrick",     "Catherine", "Alexander", "Carolyn",   "Jack",
  ...
  for (auto &name : names) {
    for (auto &surname : surnames) {
      ...
      std::string person_str =
          "insert " + std::to_string(AliceDB::get_current_timestamp()) + " " +
          name + " " + surname + " " + std::to_string(age);
  ...
  AliceDB::Producer<Person> *prod =
      new AliceDB::FileProducer<Person>(file_name, parseLine);
  ...
  AliceDB::Node *name_getter =
      new AliceDB::ProjectionNode<Person, Name>(adults, proj_names);
  ...
  /*
     ...
     auto fun_fields = [](Person *P, Person *P2){ std::memcpy(P2,P,
     sizeof(Person)); }; auto join_layout = [](Person *left_p, Person *right_p ,
     Person *out ){  std::memcpy(out, left_p, sizeof(Person)); };

     AliceDB::Node *join = new AliceDB::JoinNode<Person, Person, Person,
     Person>(unn,unn,  fun_fields, fun_fields, join_layout);
 */
}